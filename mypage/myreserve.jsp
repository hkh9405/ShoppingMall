...
<link rel="stylesheet" type="text/css" href="<%=cp %>/project3//mypage/css/mp_reserve.css"/>
...
function comma() {
	...
	var size = ${mlists.size() };
	var str2 = document.getElementById('commaT').value;
	var str3 = str2.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");

	document.getElementById('commaTT').innerHTML = str3  + " 원";

	for (i = 0; i < size; i++) {
		var str = document.getElementById('comma' + i).value;

		var str1 = str.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
		document.getElementById('cMileage' + i).innerHTML = str1;
	}
}
...
<div id="content">
	...
	<div id="myReserv">
		<div class="page-body">
			<p class="t-box-msg">
			<strong>${sessionScope.customInfo.name }</strong>님이 쇼핑몰에서 사용 가능한 적립금 내역입니다.
			</p>
			<div class="att-box-wrap">
				<div class="att-box">
					<div class="table-att">
					...
					<tr>
						...
						<th scope="row"><div class="tb-left">총 보유 적립금액</div></th>
						
						<td>
							<input type="hidden" id="commaT" value="${totalMileage }">
							<div id="commaTT" class="tb-right"></div></td>
						<th scope="row"><div class="tb-left"></div></th>
						<td><div class="tb-right"></div></td>
					</tr>
					</tbody>
					</table>
					</div>
			...
			<div class="table-d2-list">
				...
				<tr>
					...
					<th scope="row"><div class="tb-center">적립내역</div></th>
				...
				<c:when test="${!empty mlists }">
					...
					<tr>
						...
						<td><div class="tb-left">${dto.content }</div></td>
						<td>
							<input type="hidden" id="comma${i }" value="${dto.mileage }">
							<div class="tb-right tb-bold"><span id="cMileage${i }" class="fc-red"></span> 원</div>
						</td>
					</tr>		
